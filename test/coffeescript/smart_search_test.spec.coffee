...
describe "search query parser", ->
  ...
  it "should identify the key and partial term correctly for a query term with a colon", ->
    expect(@parsePartialQuery("foo:bar")).toEqual { key: "foo", partialValue: "bar" }
    expect(@parsePartialQuery("foo:bar ")).toEqual { key: "", partialValue: "" }

  it "should ignore extra colons afterwards", ->
    expect(@parse("foo:bar:baz")["foo"]).toEqual "bar:baz"
    expect(@parsePartialQuery("foo:bar:baz")).toEqual { key: "foo", partialValue: "bar:baz" }

  it "should parse an empty key to path", ->
    expect(@parse(":foo")["paths"]).toEqual(["foo"])
    expect(@parsePartialQuery(":foo")).toEqual { key: ":foo", partialValue: "" }
  ...
  it "should allow for a comma-separated list, including spaces", ->
    ...
    expect(@parsePartialQuery("repos:db, barkeep,coffee")).toEqual { key: "repos", partialValue: "coffee" }
  ...
  it "should allow for spaces after the colon in a search term", ->
    ...
    expect(@parsePartialQuery("repos: barkeep authors: caleb")).toEqual { key: "authors", partialValue: "caleb" }
  ...
  it "should gracefully handle a trailing comma", ->
    expect(@parse("foo:bar,baz,")["foo"]).toEqual "bar,baz"
    expect(@parsePartialQuery("foo:bar,baz,")).toEqual { key: "foo", partialValue: "" }


  it "should allow for using paths like any other key", ->
    expect(@parse("paths: foo, bar,baz")["paths"]).toEqual ["foo,bar,baz"]
    expect(@parsePartialQuery("paths: foo, bar,baz")).toEqual { key: "paths", partialValue: "baz" }
  ...
  it "should allow for setting paths by not specifying a key", ->
    ...
    expect(@parsePartialQuery("foo bar baz repos:blah paths:some/path")).toEqual
      key: "paths"
      partialValue: "some/path"
  ...
  it "should handle sha plus another search term", ->
    ...
    expect(@parsePartialQuery("0e7d9bd repos:barkeep")).toEqual { key: "repos", partialValue: "barkeep" }
  ...
  it "should not confuse words for sha", ->
    ...
    expect(@parsePartialQuery(word)).toEqual({ key: word, partialValue: "" }) for word in sampleWords
  ...
  it "should allow for some synonyms instead of the intended keywords", ->
    for keyword, synonym of { branches: "branch", authors: "author", repos: "repo" }
      ...
      expect(@parsePartialQuery("#{synonym}: foobar")).toEqual { key: keyword, partialValue: "foobar" }
