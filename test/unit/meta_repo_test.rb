...
class MetaRepoTest < Scope::TestCase
  ...
  context "grit_commit" do
    should "return nil for invalid repos and commits" do
      assert_equal nil, MetaRepo.instance.grit_commit(@repo_name, "non_existant_sha")
      assert_equal nil, MetaRepo.instance.grit_commit("invalid_repo", @first_commit)
      assert_equal @first_commit, MetaRepo.instance.grit_commit(@repo_name, @first_commit).id_abbrev
    end
  end

  context "search_options_include_commit" do
    should "find a commit by author" do
      assert_equal false, MetaRepo.instance.search_options_match_commit?(@repo_name, @first_commit,
          { :authors => ["Jones"] })
      assert_equal true, MetaRepo.instance.search_options_match_commit?(@repo_name, @first_commit,
          { :authors => ["Phil"] })
      assert_equal true, MetaRepo.instance.search_options_match_commit?(@repo_name, @first_commit,
          { :authors => ["Phil", "Jones"] })
    end
    ...
    should "find a commit by branch" do
      ...
      assert_equal false, MetaRepo.instance.search_options_match_commit?(@repo_name, commit_on_branch,
          { :branches => ["nonexistant_branch"] })
      assert_equal true, MetaRepo.instance.search_options_match_commit?(@repo_name, commit_on_branch,
          { :branches => ["cheese"] })
    ...
    should "return false for a commit which has matching commits in its history, but does not itself match" do
      ...
      assert_equal false, MetaRepo.instance.search_options_match_commit?(@repo_name, second_commit,
          { :paths => ["units.txt"] })
    end
  end
end
